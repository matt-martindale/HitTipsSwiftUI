...
struct PersonaCardView: View {
        ...
        ZStack(alignment: .topTrailing) {
            ...
            if persona.isPremium {
                Image(systemName: "crown.fill")
                    ...
                    .foregroundStyle(.yellow.opacity(isSelected ? 1.0 : 0.8))
                    .background(
                        Circle()
                            .fill(isSelected ? Color.white : .clear)
                            .frame(width: 28, height: 28)
                    )
                    .offset(x: 12, y: -12) // push outside card bounds
            }
        }
    ...
    private func fillColor() -> AnyShapeStyle {
        if persona.isPremium {
            return isSelected
            ? AnyShapeStyle(LinearGradient(colors: [.purple.opacity(0.8), .blue.opacity(0.8)],
                                           startPoint: .topLeading,
                                           endPoint: .bottomTrailing))
            : AnyShapeStyle(LinearGradient(colors: [.purple.opacity(0.2), .blue.opacity(0.2)],
                                           startPoint: .topLeading,
                                           endPoint: .bottomTrailing))
        } else {
    ...
    private func backgroundColor() -> AnyShapeStyle {
        if persona.isPremium {
            return isSelected
            ? AnyShapeStyle(LinearGradient(colors: [.purple.opacity(0.3), .purple.opacity(0.3)],
                                           startPoint: .topLeading,
                                           endPoint: .bottomTrailing))
            : AnyShapeStyle(LinearGradient(colors: [.purple.opacity(0.2), .blue.opacity(0.2)],
                                           startPoint: .topLeading,
